...
void tri_bulle(int n)                                                   //Fonction de tri à bulle ( Se ref. wikipedia tri a bulle)
{
    ...
    typec--;
    n--;
    typeap=(((float)typea/(float)n)*100);                               //Calcul de pourcentage pour agrémenter l'affichage des statistiques
...
{
    char chaine[255] = "r";                                  //Variable chaine définie avec une taille de 255 qui contiendra la chaine en cours d'utilisation
...
int main (int agrc, char * argv[])
{
    ...
    char Exiasaver_home[128];                                   //Char recupererant les variables globales
    char Exiasaver1_Pbm[128];                                                    //*****//
    char Exiasaver2_Pbm[128];                                                   //*****//
    int Exiasaver2_taille;                                      //Int recuperant la varibales globales pour plus bas
    int Exiasaver2_sleep;                                       //Int recuperant la varibales globales pour plus bas
    char Exiasaver2_taille_c[128];                                              //*****//
    char Exiasaver2_sleep_c[128];                                               //*****//
    char Exiasaver3_Pbm[128];                                                   //*****//
    char Exiasaver32_Pbm[128];                                  //Char recupererant les variables globales

    int positionx;                                              //Int qui ont les positions en x et y de l'avion a son lancement
    int positiony;
    int sens;                                                   //Int qui donne le sens de l'avion a son lancement, 0 1 2 ou 3

    char positionxc[5];                                         //Char qui ont les positions en x et y de l'avion a son lancement, pour les paramètres
    char positionyc[5];
    char sensc[10];                                             //Char qui donne le sens de l'avion.....

    char arghaut[128];                                          //Chemins vers toutes les images utilisées, nuage et sens de l'avion
    char argbas[128];
    char arggauche[128];
    char argdroit[128];
    char argnuage[128];
    ...
    {
        if (getenv("EXIASAVER_HOME")!=NULL) strcpy(Exiasaver_home,getenv("EXIASAVER_HOME"));        //On recupère chaque variable environnement et on les met dans des char
        ...
        if (random==0)                                          //Si le nombre genere est 0 alors...
        {
            ...
            if (f==0)                                           //Uniquement le processus fils avec un PID de 0 peut entrer dans la boucle
            {
                ...
                switch(randomizing){
                    case 0:
                        strcpy(data,"nuage.pbm");               //On met la chaîne de caractère grenouille.pbm dans la variable data si random vaut 0
                        break;
                    case 1:
        ...
        if (random==1)
        {
            ...
            if (f==0)
            {
                if(Exiasaver2_taille==1) strcpy(data,"5x3");                         //On rentre dans data des valeurs de la taille de l'horloge
                ...
                strcat(Exiasaver_home,fichier);                                     //On colle le chemin au nom du fichier pour otebnir le chemin complet

                sprintf(Exiasaver2_taille_c,"%d",Exiasaver2_taille);                //On transforme la taille (int) en char pour le donner en paramètre
                sprintf(Exiasaver2_sleep_c,"%d",Exiasaver2_sleep);                  // //

                strcat(data,"/");                                                   //On ajoute un slash a la suite de data pour formater clairement l'affichage dans l'historique
                strcat(data,Exiasaver2_sleep_c);                                    //On ajoute ensuite le temps de refresh a la suite du data
                entreehistorique(random,data);                                      //On envoie data et random dans la fonction de l'entrée dans l'histoique 
                
                printf("Demarrage du dynamique\n");
                execl(Exiasaver_home,fichier,Exiasaver2_taille_c,Exiasaver2_sleep_c,Exiasaver2_Pbm,NULL);               //On execute le fichier avec les arguments
                exit(1);
            }
            else
            {
